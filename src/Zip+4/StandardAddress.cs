...
{
    /// <summary>
    ///     The Delivery Address Line, as matched against the ZIP+4 file, is
    ///     broken down into its individual components on the mailpiece with one space
    ///     between address elements.
    /// </summary>
    public class StandardAddress : PostalAddress
    {
        ...
        protected override string Parse(List<Match> collection)
        {
            if (collection.Count > 0)
            {
                ...
                {
                    this.Street = Address.Parse<Highway>(reverse);
                    this.Number = Address.Parse<Number>(reverse);
                }
                else
                {
                    this.PostDirectional = Address.Parse<Direction>(reverse);
                    this.Suffix = Address.Parse<Suffix>(reverse);
                    this.Street = Address.Parse<Street>(reverse);
                    this.PreDirectional = Address.Parse<Direction>(reverse);
                    this.Fraction = Address.Parse<Fraction>(reverse);
                    this.Number = Address.Parse<Number>(reverse);
                }
            }
        ...
        {
            var reverse = collection.Cast<Match>().Reverse().ToList();
            return (Address.Parse<Number>(reverse) != null || collection.Any(m => Highway.Is(m.Value)));
        }
